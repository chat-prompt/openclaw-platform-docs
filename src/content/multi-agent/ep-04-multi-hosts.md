...
# 03 · 사무실을 두 곳으로 — 맥미니 A + B에서 4마리 키우기

> 한 사무실(맥미니 A)에 두 마리 잘 살고 있는데, 일이 늘어나서 **사무실 한 곳을 더 열기**로 했어.
> 두 사무실의 직원들이 같은 슬랙으로 소통하고, **공용 자료실**(git 레포)을 양쪽에서 같이 보는 구조.
> 예시:
>   - 🏢 맥미니 A: 뽀야(팀장) + 뽀짝이(부팀장)
>   - 🏢 맥미니 B: 뽀둥이(데이터 전담) + 뽀식이(고객응대 전담)
>   - 📚 공용 자료실(`bbopters-shared` git 레포): 양 사무실에서 clone. 스킬·hook·팀 문서·템플릿을 같이 씀

## 이 문서가 해주는 거

같은 슬랙에 4마리 봇이 출근해 있는데, 그중 2마리는 사무실 A에서, 나머지 2마리는 사무실 B에서 일하는 분산 구조를 셋업한다. 핵심 3가지:
...
- [ ] 1마리 / 2마리 가이드 ([ep.2](./ep-02-single-agent), [ep.3](./ep-03-two-agents-same-host)) 끝낸 상태
- [ ] 새 머신(맥미니 B 등) 한 대 추가 가능
...
- **🛡️ 장애 격리**: A 사무실 정전나도 B의 뽀둥이·뽀식이는 멀쩡히 일함
- **⚖️ 리소스 분산**: 메모리 많이 먹는 직원은 별도 사무실로
- **🎯 도메인 분리**: B는 데이터 분석용 고사양, A는 일반 업무용 저사양
...
규칙: **봇 1마리 = 사무실 1곳**. A에서 뽀야 돌리면 B에선 절대 같은 토큰으로 띄우지 말 것.
...
- 맥미니 B `openclaw.json`: 뽀둥이·뽀식이만 정의, 둘 중 1명에게 `"default": true`

⚠️ A의 뽀야와 B의 뽀둥이가 둘 다 `default: true`여도 충돌 없음 — 슬랙이 토큰별로 알아서 분리 배달하니까. **머신 내부 폴백**만 결정.
...
사무실 N개가 같은 자료실을 공유하니까 A에서 수정·push한 걸 B가 pull 안 하면 버전 불일치 사고. **스킬도 마찬가지** — 오래된 스킬로 작업하면 스키마 변경분 놓쳐.
...
> 📚 **비유** — 새 사무실(맥미니 B) 차리기 전에 가장 먼저 할 일. 사무실에서 일하려면 팀 자료실(스킬·hook·문서)이 책상 옆에 있어야 함. 자료실은 **모든 사무실에서 똑같은 경로**로 두는 게 핵심.
...
## STEP 1 · 맥미니 B 준비 — "새 사무실 인테리어"

> 🏢 **비유** — 새 사무실에 책상·전화기·우편함 깔아주기. 사무실 운영체제(OpenClaw 게이트웨이) 깔고 자동 출근(launchd) 등록하는 단계.
...
> 💡 **이 섹션이 발표용 핵심** — 사무실이 둘로 나뉘면 직원들끼리 어떻게 일을 주고받을까? 직통 전화(subagent 위임)는 안 되니까 슬랙·자료실·팀 위키를 활용하는 4가지 패턴.
