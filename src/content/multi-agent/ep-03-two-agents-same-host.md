...
4. **우편물 분류는 accountId 기준** — 한 직원이 여러 슬랙 account 받을 수도 있어 (예: 뽀야가 default/dajidongsan/hbscom 전부 받는 식).
...
- **폴백 모델 차등** — 뽀야는 비워두고(`[]`), 뽀짝이만 codex 폴백 켬. 왜냐면:
  - 뽀야는 집사 판단에 직접 쓰이는 팀장이라 **톤 뒤틀리면 더 위험** → 차라리 침묵
  - 뽀짝이는 AI스터디 발송·운영 실무 담당이라 **멈추면 수강생 피해** → 톤 좀 흐려도 작업 이어가기
...
> Slack `default`/`dajidongsan`/`hbscom` account로 온 메시지는 다 뽀야 책상으로.
> Slack `bbojjak` account로 온 메시지는 뽀짝이 책상으로.
> (한 직원이 여러 account 동시에 받을 수도 있어 — 뽀야는 슬랙 워크스페이스 3개를 한 책상에서 처리)
