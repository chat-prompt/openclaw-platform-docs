...
```
~/.openclaw/
├── workspace-bboya/      ← 뽀야 책상 (성격설정서 6장이 여기 깔려있음)
├── workspace-bbojjak/    ← 뽀짝이 책상
└── workspace-arongi/     ← 아롱이 책상
```

봇한테 일 들어오면 OpenClaw가 **그 봇 책상(cwd)에서 `claude`를 실행**해. 그러면 그 책상에 깔린 성격설정서(IDENTITY/SOUL/AGENTS 등)를 Claude가 자동으로 읽어서 그 봇 페르소나로 답함. **이게 멀티에이전트의 핵심 트릭** — 같은 `claude` 바이너리인데 어느 책상에서 실행되냐에 따라 다른 봇이 됨.
