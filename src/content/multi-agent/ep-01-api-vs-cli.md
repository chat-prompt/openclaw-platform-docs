...
## 🤔 잠깐 — 그냥 터미널에서 `claude` 쓰면 되지, 왜 OpenClaw가 필요해?

이게 **가장 먼저 풀어야 할 질문**이야. 이거 안 풀리면 뒤에 나오는 게 다 "왜 이렇게 복잡하게 해?"가 되거든.

### Claude Code만 단독으로 쓰면 = "내 컴퓨터 앞에 앉은 1인 비서"

내가 노트북 켜고 터미널 열고 `claude`라고 치면 Claude가 열려. "코드 고쳐줘", "이 파일 분석해줘" 해서 일 시키지. 근데:

```
🙋 사람: 노트북 앞에 앉음 → 터미널 켬 → `claude` 입력 → 대화
🤖 Claude: 그 터미널 안에서만 작동
```

한계가 빤해:

- **사람이 옆에 있어야 작동** — 노트북 닫으면 끝
- **1:1 대화만 가능** — 다른 채널(슬랙/카톡/텔레그램) 연결 X
- **여러 페르소나 동시 운영 X** — 한 터미널엔 한 Claude
- **24시간 응답 X** — 자고 있을 때 들어온 메시지 못 받음
- **팀이 같이 못 씀** — 내 노트북에서만 도는 거니까

비유로: 휴대폰에 비서 앱 깔아둔 거. **내가 앱 켤 때만 작동**.

### OpenClaw로 감싸면 = "비서를 회사에 출근시킨 격"

OpenClaw가 Claude Code를 백그라운드로 24시간 돌려주고, 슬랙·텔레그램 같은 채널과 연결해주는 **사무실 + 안내데스크** 역할:

| Claude Code 단독 | OpenClaw + Claude Code |
|---|---|
| 사람이 터미널 켜야 작동 | 24시간 자동 백그라운드 |
| 1:1 대화만 | 슬랙·텔레그램·카톡·웹훅 동시 연결 |
| 한 페르소나만 | 여러 봇 동시 운영 (뽀야·뽀짝이 등) |
| 페르소나 매번 직접 알려줘야 | 워크스페이스 파일 자동 주입 |
| 대화 끊기면 맥락 잃음 | 슬랙 스레드 맥락 자동 복구 (hook) |
| 내 노트북에서만 | 팀 전체가 슬랙으로 같이 씀 |
| 토큰·인증 직접 관리 | 봇별 OAuth 격리 |

### 한 줄로

> 🐱 **Claude Code 단독** = 휴대폰 비서 앱 (내가 켤 때만)
> 🏢 **OpenClaw + Claude Code** = 비서가 사무실 출근 (24시간, 팀 전체가 슬랙으로 일 시킴)

즉 OpenClaw는 **"AI 비서를 진짜 팀원처럼 만들어주는 운영체제"**. Claude Code는 그 비서의 뇌고. 이 시리즈는 그 뇌를 슬랙으로 출근시키는 셋업 가이드야.
