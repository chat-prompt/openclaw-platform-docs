...
#### 시나리오 2️⃣ — Claude Code + 슬랙 채널 플러그인 (직결)

> 사실 Claude Code엔 **채널(Channel) 플러그인** 시스템이 있어. 공식적으로 텔레그램·디스코드 플러그인이 있고, 슬랙은 [집사가 직접 만든 플러그인](https://github.com/daht-mad/claude-code-slack-channel)이 있음.

```
슬랙 → Socket Mode → MCP 서버(server.ts) → 단일 Claude Code 세션 → 답장
```

- 슬랙에 답하긴 함 ✅
- 근데 **1세션 = 1작업** — A 작업 중일 때 B 들어오면 큐잉도 안 됨
- **터미널이 곧 생명줄** — 맥 잠들거나 터미널 닫히면 죽음
- 컨텍스트 길어지면 점점 느려짐. 세션 리셋하면 맥락 다 날아감
- 보안(`Bash(*)` 같은 권한)이 다 사용자 책임
- Agent Teams(한 세션 내 멀티봇)는 실험 기능 — 안정성 ↓

비유: 1인 프리랜서가 슬랙으로 일 받는 거. **혼자 받으니까 동시에 두 명 못 받음**.
...
| 항목 | 🟦 단독 터미널 | 🟪 클코+슬랙 직결 | 🟢 OpenClaw+클코 |
|---|---|---|---|
| 슬랙 연결 | ❌ | ✅ | ✅ |
| 24시간 자동 가동 | ❌ | ⚠️ 터미널 유지 필수 | ✅ 데몬 |
| 동시 처리 | ❌ | ❌ 1세션 1작업 (큐잉 X) | ✅ N채널 병렬 |
| 여러 봇 페르소나 | ❌ | ⚠️ Agent Teams (실험) | ✅ cwd 격리로 안정 |
| 페르소나 자동 주입 | ❌ | ⚠️ 직접 박아야 | ✅ 워크스페이스 파일 자동 |
| 스레드 맥락 자동 복구 | ❌ | ⚠️ 일부 (직접 fetch) | ✅ hook 자동 |
| 크론·웹훅 자동화 | ❌ | ❌ | ✅ 내장 |
| 모델 폴백 (장애 시) | ❌ | ❌ | ✅ Codex 폴백 |
| 보안·권한 관리 | 사용자 책임 | 사용자 책임 | OpenClaw가 게이트웨이로 통제 |
| 파일 직접 수정 능력 | ✅ 강함 | ✅ 강함 | ⚠️ API 경유 (Bash 사용 시 제한) |
| 세팅 난이도 | 가장 쉬움 | 중간 | 가장 높음 (이 시리즈가 풀어줌) |

### 그래서 우리는 둘 다 쓴다 — 역할 분담

> 🐱 **OpenClaw 봇** = 메인 비서 (24시간 슬랙으로 일 받음)
> 🤖 **Claude Code 직결 봇** = 보조 모드 (파일 작업·코드 수정 같은 무거운 로컬 작업)

즉 **"OpenClaw가 더 좋다"가 아니라 "역할이 다르다"**가 정답. 다만 *"AI 비서를 진짜 팀원처럼"* 만드는 건 OpenClaw 영역. 이 시리즈는 그쪽 셋업을 다뤄.
