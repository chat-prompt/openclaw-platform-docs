...
tags: ["OpenClaw", "아키텍처", "작동 원리", "cwd", "--resume", "Claude CLI"]
---

# 3 · OpenClaw 작동 흐름 — 메시지 한 줄이 페르소나 입은 답이 될 때까지
...
>
> 📎 *이 글은 [Slack × Claude CLI 멀티에이전트 시리즈 ep.2](../multi-agent/ep-03-anatomy)와 동일 본문이야. 두 시리즈 모두에 어울리는 글이라 양쪽에 묶어뒀어.*
...
이 흐름의 한 단계 안으로 더 들어가자 — 워크스페이스 안의 *스킬*은 어디에 박아야 자동 인식되는가? → [ep.4 스킬은 어디에 박지](./ep-04-skills-resolution)

또는 멀티에이전트 시리즈 흐름을 따라가고 싶으면 → [multi-agent ep.4 1마리 셋업](../multi-agent/ep-04-single-agent)
