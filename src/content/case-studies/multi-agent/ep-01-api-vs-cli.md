...
**그 외 한계**: DM X (채널만), GitHub repo 필수, 1세션 1PR, Claude Code on the web 액세스 필수, 슬랙만 (텔레그램·웹훅 X)
...
- **여러 채널 동시** — 슬랙·텔레그램·웹훅 한 시스템에서 (메인은 슬랙)
...
| 슬랙 외 채널 (텔레그램·웹훅) | ❌ | ❌ 슬랙만 | ✅ 다채널 지원 (메인은 슬랙) |
...
- **"뽀야"라는 팀 공용 페르소나 봇**을 슬랙(메인)·텔레그램·웹훅으로 두루 쓰고 싶음 → OpenClaw 영역. 이 시리즈가 그쪽 셋업.
