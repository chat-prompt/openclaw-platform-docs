...
> 🐱 **OpenClaw 봇 (뽀야/뽀짝이...)** = 회사 직원처럼 출근하는 팀 공용 봇. 페르소나·기억·스킬 보유
> 💬 **Claude in Slack** = 각자 자기 Claude로 슬랙에서 코딩 위임 (집 도구를 사무실로 들고 옴)
> 🤖 **Claude Code 단독 터미널** = 집에서 무거운 코딩 (파일·코드 직접 수정)

**OpenClaw의 정체성**: 개인 도구를 슬랙에 들고 오는 게 아니라, **에이전트가 진짜 동료가 되어 회사 슬랙에서 팀원으로 협업하는 것**. 이름·성격·기억·스킬을 가지고 24시간 출근. 이 시리즈는 그 모델을 어떻게 만드는지 풀어줘.
