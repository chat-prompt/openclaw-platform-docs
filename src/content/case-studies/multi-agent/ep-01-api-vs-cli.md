...
> 셋업 들어가기 전 이 큰 그림을 잡고 가야 [ep.2 작동 흐름](./ep-02-anatomy)부터 [ep.5 N대 머신](./ep-05-multi-hosts)까지 흐름이 풀려.
> 비개발자도 따라올 수 있게 비유 위주로 풀었어.
>
> 📜 **"어쩌다 지금 셋업이 됐는지" 변천사가 궁금하면** → [ep.0 OpenClaw 변천사](./ep-00-journey) (토큰 빌려쓰기 → ACP 우회 → Claude CLI 정식). 본 편은 **지금 표준**을 다루고, 거쳐온 길은 0편으로 분리했어.
...
> 🔍 **OpenClaw 골랐어도 끝이 아니다** — 그 안에서 *PI 모드(빌트인) vs Claude CLI 모드* 두 갈래가 또 있어. 우린 CLI를 골랐고, 그 결정으로 *잃는 것·얻는 것*이 분명히 갈려. 공식문서 근거로 정리한 심화 편을 따로 뒀어 → **[ep.1.5 PI vs Claude CLI — OpenClaw 안의 두 갈래](./ep-01a-pi-vs-cli)**
>
...
>
> → **[ep.2 OpenClaw 작동 흐름 — 메시지 한 줄이 페르소나 입은 답이 될 때까지](./ep-02-anatomy)**
>
...
> 📜 변천사가 궁금하면 → [ep.0 OpenClaw 변천사 (토큰 빌려쓰기 → ACP → Claude CLI 정식)](./ep-00-journey)

## 다음 단계

자, 이제 셋업 들어가자 → [ep.2 OpenClaw 작동 흐름](./ep-02-anatomy)부터 보고 [ep.3 1마리 출근시키기](./ep-03-single-agent)로
