...
accentColor: "#8B5CF6"
description: "@뽀야야 한 줄이 어떻게 페르소나 입은 답이 되어 돌아오는지 백엔드 흐름을 친절히 풀어보는 편. cwd가 페르소나를 만들고 --resume이 기억을 잇는 진짜 작동 원리."
...
> 🐱 **이 편의 핵심** — 같은 클로드 모델인데 *왜 봇마다 다르게 답할까?* 백엔드에서 진짜로 무슨 일이 일어나는지 8단계로 친절히 풀어볼게. 끝까지 읽으면 **cwd가 페르소나를 만든다**는 멀티에이전트의 핵심 트릭이 잡혀.

---

## 🤔 같은 클로드인데 왜 봇마다 다르지?

슬랙에서 `@뽀야야`라고 부르면 반말로 답하고, `@뽀짝아`라고 부르면 존댓말로 답해. 같은 Claude 모델·같은 `claude` 바이너리·같은 컴퓨터에서 도는데, *어떻게 이렇게 다른 페르소나가 가능할까?*

답은 한 단어에 있어 — **cwd**. 어느 폴더에서 클로드가 깨어났느냐가 페르소나를 만들거든. 어떻게 그게 가능한지 *백엔드 흐름*을 따라가보자.
...
> 같은 Claude 모델인데 봇마다 다른 페르소나로 답하는 이유는 **cwd가 다르기 때문**. OpenClaw가 매 요청마다 *워크스페이스의 페르소나 파일을 시스템 프롬프트에 통째로 임베드*해서 클로드 CLI에 넘기고, 같은 슬랙 스레드의 다음 메시지는 **`--resume`**으로 이전 대화를 이어붙여. 폴백 없이 Pro/Max 구독 안에서만 답하는 게 우리의 빌링 안전장치야.
