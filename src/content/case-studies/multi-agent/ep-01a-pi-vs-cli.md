...
### 3. 스트리밍·정형 출력이 CLI 포맷에 묶임

> 📖 **공식문서 발췌 (번역)** — `gateway/cli-backends.md` (Limitations)
>
> "**스트리밍은 백엔드별이다.** 어떤 백엔드는 JSONL 스트리밍을 하고, 다른 백엔드는 종료 시까지 버퍼링한다."
> "**정형 출력은 CLI의 JSON 포맷에 의존한다.**"

OpenClaw가 직접 응답 파싱하는 PI와 달리, CLI 모드는 *그 CLI가 뱉는 JSON/JSONL*을 OpenClaw가 받아서 처리. 즉 응답 구조 변경권이 CLI 손에 있음.

### 4. 공식문서가 CLI 백엔드를 *폴백용*으로 설계했다

> 📖 **공식문서 발췌 (번역)** — `gateway/cli-backends.md` (도입부)
>
> "OpenClaw는 로컬 AI CLI를 **API 제공자가 다운되거나, rate-limited되거나, 일시적으로 오작동할 때** 텍스트 전용 *폴백*으로 실행할 수 있다. 이는 의도적으로 보수적이다. (...) **이는 안전망(safety net)으로 설계되었으며, 주력 경로(primary path)가 아니다.**"

→ **공식 권장 패턴은 PI(또는 API 키)이고, CLI는 폴백.** 우리가 CLI를 *주력*으로 굴리는 건 OpenClaw 설계 의도와 *반대 방향*. 그만큼 *왜 그렇게 굴리는지* 명확한 이유가 있어야 해 — 우린 Pro/Max 정액 + Claude Code 훅 생태계 둘 다 잡으려고.
...
| **스트리밍·정형 출력** | OpenClaw가 직접 제어 | CLI JSON 포맷에 의존 |
| **OpenClaw 플러그인 훅** | 풀 동작 | 일부 제한 (CLI 자체 도구는 외부) |
| **공식 권장 위치** | 주력 (primary) | 폴백 (safety net) |
