...
> 📌 **같은 머신 안 협업이 궁금하면** → [ep.3 STEP 6 작동법](./ep-03-two-agents-same-host) 참조. 멘션·sessions_send·위임 spawn 3가지 방식이 거기.
...
여기까지가 4편 시리즈의 끝. 1마리(ep.2) → 같은 머신 2마리(ep.3) → 여러 머신 N마리(ep.4)로 확장하면서 셋업과 작동법을 한 호흡에 풀었어.

| 편 | 핵심 |
|---|---|
| [ep.1 변천사](./ep-01-api-vs-cli) | 왜 Claude CLI인가 + OpenClaw 등장인물 8명 |
| [ep.2 1마리](./ep-02-single-agent) | 1마리 셋업 + 작동법(멘션) |
| [ep.3 2마리](./ep-03-two-agents-same-host) | 2마리 셋업 + 작동법(멘션·sessions_send·위임) |
| **ep.4 N마리** (이 편) | N대 머신 셋업 + 공용 레포 규칙 + 머신 걸친 협업 4가지 |
