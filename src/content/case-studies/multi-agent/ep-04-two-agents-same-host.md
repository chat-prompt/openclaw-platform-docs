...
# 03 · 같은 사무실에 짝꿍 한 마리 더 들이기 — 뽀야 + 뽀짝이
...
- [ ] **1마리는 이미 돌고 있음** ([ep.3 가이드](./ep-03-single-agent) 끝낸 상태)
...
| 위임이 머신 걸쳐 안 됨 | `subagents.allowAgents`는 같은 머신 안에서만 작동 | [ep.5 협업 패턴 1(슬랙 스레드)](./ep-05-multi-hosts) 사용 |
...
물리 머신을 여러 대로 분산하고 싶으면 → [ep.5 여러 물리 머신에 여러 마리](./ep-05-multi-hosts)
